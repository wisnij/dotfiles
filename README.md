...
Configuration files and user scripts that I use on one or more of my machines.
Feel free to copy and/or adapt any of these for personal use.

These are intended to be installed and managed with [GNU
...
- All of my packages can exist in the same branch.
  Moving a file from one package to another is as simple as `git mv`.
  I can search across all packages with a single `git grep`.
- Cloning a repo and installing it are two separate steps, so all the files for
  all packages can be synced locally before running the install steps.
  This is occasionally relevant for configuration files that modify how network
...
- vcsh overlays all of its managed files into your home directory as the working
  ...
  tool.
  Stow creates symlinks to the repo files, which is immediately obvious with a
  simple `ls -l` and can be discerned from the file itself, without having to
  reference a git repo somewhere else.
  It also makes it easy to see which files in a directory are Stow-managed and
  which are not, and the specific package each file is associated with is
  visible in the symlink path.
...
- vcsh overlays the _entire_ repo file tree into your home dir, so the only
  ...
  With Stow, the installable packages are in individual subdirectories, so files
  not associated with any of them can go at the repo root or elsewhere.
